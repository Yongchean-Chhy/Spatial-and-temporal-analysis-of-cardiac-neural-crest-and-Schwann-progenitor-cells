...
# =========================
# Merge cNCC and Schwann ST spatial maps
# =========================
...
axes[0].set_title("cNCC-like signature")
...
axes[1].set_title("Schwann-like signature")
axes[1].set_xlabel("Spatial x")
axes[1].set_ylabel("Spatial y")
axes[1].set_aspect("equal", adjustable="box")
fig.colorbar(sc2, ax=axes[1], fraction=0.046, pad=0.04)

plt.suptitle("Spatial Abundance of cNCC-like and Schwann-like Signatures", fontsize=14)
plt.tight_layout()
plt.savefig("ST_cNCC_Schwann_merged_map.png", dpi=300, bbox_inches="tight")
plt.close()

print("Saved: ST_cNCC_Schwann_merged_map.png")

# pick top marker genes
marker_genes = [
    "SST", "PRPH", "NEFL", "NTRK3",
    "TTYH1", "SPHK1", "LAMB2", "MIA"
]

marker_genes = [g for g in marker_genes if g in sub_expr.columns]

heatmap_data = sub_expr[marker_genes].copy()
heatmap_data["Subtype"] = sub_meta["predicted_subtype"].values

heatmap_avg = heatmap_data.groupby("Subtype").mean().T

heatmap_avg = heatmap_avg.rename(columns={
    "Cardiac neural crest-like": "cNCC-like",
    "Schwann progenitor-like": "SPC-like"
})

heatmap_avg_log = np.log1p(heatmap_avg)
...
plt.title("Distinct Marker Signatures")
plt.xlabel("")
plt.ylabel("")

plt.tight_layout()
plt.savefig("marker_gene_heatmap_log.png", dpi=300)
plt.show()